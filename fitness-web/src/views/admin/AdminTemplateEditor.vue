...
/**
 * 管理后台 - 计划模板编辑器
 * 管理员创建/编辑系统级模板，支持完整的训练日、动作、餐次配置
 * 与"我的计划"编辑器功能一致，确保导入模板时无缝衔接
 */
...
import { createAdminTemplate, updateAdminTemplate, getAdminTemplateDetail } from '@/api/admin'
...
async function fetchDictData(): Promise<void> {
  const [planTypes, splitTypes, difficultyLevels, trainingTypes, muscleGroups, mealTypes] = await Promise.all([
  ...
  if (!planTypeOptions.value.length || !splitTypeOptions.value.length ||
      !difficultyOptions.value.length || !trainingTypeOptions.value.length ||
      !muscleGroupOptions.value.length || !mealTypeOptions.value.length) {
...
function initFormData(): void {
  ...
  formData.difficulty = getPreferredOptionValue(difficultyOptions.value, 'BEGINNER')
...
function validateForm(): { valid: boolean; message: string } {
  if (!formData.templateName.trim()) {
    return { valid: false, message: '请输入模板名称' }
  }
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateAdminTemplate(editId.value, payload)
...
function populateForm(detail: any) {
  formData.templateName = detail.templateName || detail.planName || ''
  ...
  formData.difficulty = detail.difficulty || getPreferredOptionValue(difficultyOptions.value, 'BEGINNER')
...
async function loadPageData() {
  ...
  try {
    await fetchDictData()
    if (isEdit.value) {
      const detail = await getAdminTemplateDetail(editId.value) as any
      populateForm(detail)
    } else {
      initFormData()
    }
